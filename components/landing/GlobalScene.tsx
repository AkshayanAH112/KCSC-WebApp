...
function ImpactBurst({ storyRef }: { storyRef: React.RefObject<StoryRef> }) {
  ...
  const { geometry, directions } = useMemo(() => {
    ...
    // Random initial burst directions, generated once on mount (empty deps below) — not
    // render-time impurity, but the purity rule can't see through the useMemo boundary.
  ...
  // useFrame runs on the r3f render loop (rAF), outside React's render cycle — mutating
  // the geometry/material in place here is the standard, documented r3f pattern for
  // per-frame updates, not a purity violation the "components must be idempotent" rule
  // can distinguish from unsafe render-time mutation.
